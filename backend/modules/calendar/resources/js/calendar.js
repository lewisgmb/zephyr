jQuery(function ($) {
    ...
    $fullCalendar.fullCalendar({
        ...
        eventRender : function (calEvent, $event) {
            var body = calEvent.title + '<i class="delete-event icon-rt glyphicon glyphicon-trash" title="' + BooklyL10n.delete + '"></i>';
        ...
        eventAfterRender : function (calEvent, $calEventList, calendar) {
            $calEventList.each(function () {
                var $calEvent = $(this);
                var titleHeight = $calEvent.find('.fc-title').height() * 1.1;
