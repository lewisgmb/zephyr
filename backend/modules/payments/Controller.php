...
class Controller extends Lib\Controller
{
    ...
    private function createQuery( $request )
    {
        ...
        if ( ! empty( $request['sort_order'] ) &&
            in_array( $request['order_by'], array( 'created', 'type', 'customer', 'provider', 'service', 'total', 'start_date', 'status' ) )
        ) {
    ...
    public function executeGetPayment()
    {
        $data = array();
        ...
        wp_send_json_success( array( 'html' => $this->render( 'receipt', $data, false ) ) );
    }
