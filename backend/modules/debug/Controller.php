...
class Controller extends Lib\Controller
{
    ...
    public function index()
    {
        ...
        foreach ( self::getEntities() as $entity ) {
            ...
            if ( self::tableExists( $tableName ) ) {
                ...
                $debug[ $tableName ]['fields'] = array();#array_combine($tableStructure, array_fill(0, sizeof($tableStructure), 0));
    ...
    {
        $result = array();
        foreach ( scandir( AB_PATH . '/lib/entities' ) as $filename ) {
            if ( $filename == '.' || $filename == '..' ) {
                continue;
            }
            $result[] = '\\Bookly\\Lib\\Entities\\' . basename( $filename, '.php' );
        }
