jQuery(function($) {
    ...
    function submitServiceFrom($form, update_staff) {
        ...
        $.post(ajaxurl, data, function (response) {
            if (response.success) {
                ...
                $panel.find('h4.panel-title span.badge').css('background-color', response.data.color);
    ...
    function onCollapseInitChildren() {
        $('.panel-collapse').on('show.bs.collapse.bookly', function () {
            ...
            initColorPicker($panel.find('.service-color'));
            ...
            $panel.find('.ab-popover-ext').popover({
                trigger: 'hover',
                content: function () {
            ...
            $('.ab--sub-services', $panel).sortable({axis: 'y', items: "li[data-sub-service-id]"});
            ...
            $panel.find('.js-reset').on('click', function (e) {
