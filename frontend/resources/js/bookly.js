            ...
            $.ajax({
                ...
                success     : function (response) {
                    if (response.success) {
                        ...
                        // Result table initiation
                        $("#help-card-grid").on('click', '.helperSelect', function() {

                              $(".ab-select-employee").val($(this).attr('id'));

                              $( ".ab-next-step" ).trigger( "click" );



                          });

                      function SetHelperAndSubmit(RecordID) {
                          $(".ab-select-employee").val(RecordID);

                        }
                        function rowWriter(rowIndex, record, columns, cellWriter) {
                            var cssClass = "cards__item", tr;
                            if (rowIndex % 3 === 0) { cssClass += ' first'; }
                            tr = '<tr><td style="text-align: left;" class="helperSelect" id="' + record.selectValue +'">' + record.name +'</td><td style="text-align: left;">' + record.isRCR +'</td><td style="text-align: left;">' + record.isPDSB +'</td><td style="text-align: left;">' + record.talksASL +'</td><td style="text-align: left;">' + record.hasCFC +'</td><td style="text-align: left;"><input class="helperSelect" id="' + record.selectValue +'" type="radio" name="help" value="' + record.selectValue +'"> Choisir </td></tr>';
                            li = '<li class="cards__item"><div class="card helperSelect" id="' + record.selectValue +'"><div class="card__image card__image--river" style="background-image :url(' + record.avatar_url +')"></div><div class="card__content"><div class="card__title">' + record.name +'</div><p class="card__text"><strong>Certification RCR</strong> : ' + record.isRCR +'<br><strong>Principe de déplacement sécuritaire des bénéficiaires</strong> : ' + record.isPDSB +'<br><strong>Language des signes</strong> : ' + record.talkASL +'<br><strong>Attestation de vérification de casier judiciaire</strong> : ' + record.isRCR +'<br>                            </p></div></div></li>';
                            return li;
                          }

                        myRecords = [];

                        var dynatable = $('#help-card-grid').dynatable({
                          dataset: {
                            records: myRecords
                          },
                          writers: {
                        ...
                        // Result table update function
                        function UpdateStaff(staff) {
                            console.log(staff);
                          myRecords.length = 0;
                          $( "#help-card-grid" ).empty();
                          console.log(staff);
                          $.each(staff, function(i)
                            {
                                var StaffMember = staff[i];
                              for (var key in staff[i]) {
                                  var object = staff[i];
                                  if (StaffMember.hasOwnProperty(key)) {
                                      //Now, object[key] is the current value
                                      if (object[key] === null )
                                          object[key] = "Non spécifié"
                                  }
                                  if (StaffMember.hasOwnProperty(key)) {
                                      //Now, object[key] is the current value
                                      if (object[key] === null )
                              ...
                              StaffMember.name = StaffMember.name.replace('($0,00)', '');
                              myRecords.push ({
                                name:StaffMember.name,
                                isRCR : StaffMember.isRCR,
                                isPDSB :StaffMember.isPDSB,
                                talksASL :StaffMember.talksASL,
                                hasCFC :StaffMember.hasCFC,
                            ...
                            console.log('update ' + myRecords);
                            dynatable.settings.dataset.originalRecords = myRecords;
                            if ($(".ab-select-service").val()) {
                                dynatable.process();
                            }


                          console.log(myRecords)
                          var stafftable = $('#help-card-grid').data('myRecords');

                          $('#help-card-grid').dynatable({
                            dataset: {
                              records: myRecords
                            }
                          });
                        ...
                        $select_category.on('change', function() {
                            ...
                            if (category_id) {
                                ...
                                UpdateStaff(categories[category_id].staff)
                        ...
                        $select_service.on('change', function() {
                            ...
                            if (service_id) {
                                ...
                                UpdateStaff( services[service_id].staff)
