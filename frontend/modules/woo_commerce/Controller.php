...
class Controller extends Lib\Controller
{
        ...
        foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
            if ( array_key_exists( 'bookly', $cart_item ) ) {
                ...
                if ( $cart_item['quantity'] > 1 ) {
                    $userData->foreachCartItem( function ( Lib\UserBookingData $userData ) use ( $cart_item ) {
    ...
    public function paymentComplete( $order_id )
    {
        ...
        foreach ( $order->get_items() as $item_id => $order_item ) {
            ...
            if ( $data && ! isset ( $data['processed'] ) ) {
                ...
                $userData->foreachCartItem( function ( Lib\UserBookingData $userData ) use ( &$ca_list, $order_item ) {
                    if ( $order_item['qty'] > 1 ) {
                        // Equal appointments increase qty
                        $userData->set( 'number_of_persons', $userData->get( 'number_of_persons' ) * $order_item['qty'] );
                    }
    ...
    function getItemData( $other_data, $cart_item )
    {
        if ( isset ( $cart_item['bookly'] ) ) {
            ...
            $userData->setData( $cart_item['bookly'] );
            $info = array();
            $userData->foreachCartItem( function ( Lib\UserBookingData $userData ) use ( &$info ) {
