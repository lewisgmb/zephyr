...
class Controller extends Lib\Controller
{
    ...
    public function executeRenderCart()
    {
        ...
        if ( $userData->load() ) {
            ...
            $userData->foreachCartItem( function ( Lib\UserBookingData $userData, $cart_key ) use ( &$cart_items, $use_client_time_zone, $cart_columns ) {
                $slots = $userData->get( 'slots' );
                $timestamp = $slots[0][2];
                ...
                foreach ( $cart_columns as $column => $attr ) {
                    if ( $attr['show'] ) {
                        switch ( $column ) {
                            case 'service':
                                $cart_items[ $cart_key ][] = $userData->getCartService()->getTitle();
                                break;
                            case 'date':
                            ...
                            case 'time':
                                ...
                                break;
                            case 'employee':
                                $cart_items[ $cart_key ][] = $userData->getCartStaff()->getName();
                                break;
                            case 'price':
                                $cart_items[ $cart_key ][] = ( $userData->get( 'number_of_persons' ) > 1 ? $userData->get( 'number_of_persons' ) . ' &times; ' : '' ) . Lib\Utils\Common::formatPrice( $userData->getCartServicePrice() );
                                break;
                        }
                    }
                }
    ...
    public function executeSessionSave()
    {
        ...
        if ( $form_id ) {
            ...
            if ( empty ( $errors ) ) {
                // Decode slots from JSON.
                if ( isset ( $parameters['slots'] ) ) {
                    $parameters['slots'] = json_decode( $parameters['slots'] );
                }
            ...
            if ( $userData->load() ) {
                ...
                if ( $failed_cart_key === null ) {
                    ...
                    $userData->foreachCartItem( function ( Lib\UserBookingData $userData ) use ( &$ca_list, $payment_id ) {
                        $ca = $userData->save( $payment_id );
                        $ca_list[ $ca->get( 'id' ) ] = $ca;
                    } );
                    $payment->setDetails( $ca_list, $coupon )->save();
                    $response = array(
                        'success' => true,
                ...
                if ( $allow_cancel ) {
                    ...
                    if ( get_option( 'ab_settings_client_cancel_appointment_action' ) == 'delete' ) {
                        ...
                        if ( $customer_appointment->get( 'compound_token' ) ) {
                            do_action( 'bookly_compound_services_cancel_appointment', $customer_appointment );
                        } else {
        ...
        if ( get_option( 'ab_appearance_show_progress_tracker' ) ) {
            ...
            if ( ! $payment_disabled ) {
                ...
                $userData->foreachCartItem( function ( Lib\UserBookingData $userData ) use ( &$payment_disabled ) {
                    ...
                    if ( $initial_service_price === false && $userData->get( 'slots' ) === null
    ...
    private function _prepareInfoText( $step, $text, $userData )
    {
        if ( empty ( $this->info_text_codes ) ) {
            if ( $step == 1 ) {
                // No replacements.
            } elseif ( $step <= 3 || get_option( 'ab_custom_fields_per_service' ) && $step == 5 ) {
                if ( $step <= 3 ) {
                    ...
                    $service_price = $userData->getCartServicePrice();
                ...
                $this->info_text_codes = array(
                    ...
                    '[[SERVICE_TIME]]'      => '<b>' . Lib\Utils\DateTime::formatTime( $service_datetime ) . '</b>',
                    '[[SERVICE_DATE]]'      => '<b>' . Lib\Utils\DateTime::formatDate( $service_datetime ) . '</b>',
    ...
    private function _showCartButton( array $cart )
    {
        if ( get_option( 'ab_settings_step_cart_enabled' ) ) {
            $key = get_option( 'ab_custom_fields_per_service' ) ? 'custom_fields' : 'slots';
            // We need to find in cart one item with custom_fields or slots set.
