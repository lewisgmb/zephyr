...
class Updates
{
    function update_9_3()
    {
        ...
        if ( Lib\Entities\Notification::query()->where( 'type', 'client_pending_appointment_cart' )->count() == 0 ) {
            $wpdb->query( 'ALTER TABLE ' . Lib\Entities\CustomerAppointment::getTableName() . ' ADD COLUMN `payment_id` INT UNSIGNED DEFAULT NULL, ADD COLUMN `compound_service_id` INT UNSIGNED DEFAULT NULL, ADD COLUMN `compound_token` VARCHAR(255) DEFAULT NULL' );
            $wpdb->query( 'ALTER TABLE ' . Lib\Entities\Service::getTableName() . ' ADD COLUMN `type` ENUM("simple","compound") NOT NULL DEFAULT "simple", ADD COLUMN `sub_services` TEXT NOT NULL' );
            $wpdb->query( 'UPDATE ' . Lib\Entities\Service::getTableName() . ' SET `sub_services` = \'[]\'' );
            $wpdb->query( 'UPDATE ' . Lib\Entities\CustomerAppointment::getTableName() . ' SET `extras` = \'[]\' WHERE extras IS NULL' );
            $wpdb->query( 'UPDATE ' . Lib\Entities\CustomerAppointment::getTableName() . ' ca JOIN ' . Lib\Entities\Payment::getTableName() . ' p ON p.customer_appointment_id = ca.id SET ca.payment_id = p.id' );
            $this->drop_columns( Lib\Entities\Payment::getTableName(), array( 'customer_appointment_id' ) );
            $wpdb->query( 'ALTER TABLE ' . Lib\Entities\Payment::getTableName() . ' ADD COLUMN `details` TEXT' );
            $wpdb->query( 'ALTER TABLE ' . Lib\Entities\Appointment::getTableName() . ' ADD COLUMN `internal_note` TEXT' );
            $wpdb->query( 'ALTER TABLE ' . Lib\Entities\CustomerAppointment::getTableName() . ' ADD CONSTRAINT FOREIGN KEY (payment_id) REFERENCES ' . Lib\Entities\Payment::getTableName() . '(id) ON DELETE SET NULL ON UPDATE NO ACTION' );
            if ( get_option( 'ab_db_version' ) != '9.2.1' ) {
                $wpdb->query( 'ALTER TABLE ' . Lib\Entities\Service::getTableName() . ' ADD COLUMN `visibility` ENUM("public","private") NOT NULL DEFAULT "public"' );
                $wpdb->query( 'ALTER TABLE ' . Lib\Entities\Staff::getTableName() . ' ADD COLUMN `visibility` ENUM("public","private") NOT NULL DEFAULT "public"' );
            }
            $notifications = array(
                array(
                    'gateway' => 'email',
        ...
        foreach ( $appointments as $fields ) {
            ...
            if ( $payment->load( $fields['payment_id'] ) ) {
                ...
                $payment->setDetails( array( new Lib\Entities\CustomerAppointment( $fields ) ), $coupon )->save();
            }
        }
        $this->drop_columns( Lib\Entities\CustomerAppointment::getTableName(), array( 'coupon_code', 'coupon_discount', 'coupon_deduction' ) );
    }
    ...
    function drop_columns( $table, array $columns )
    {
        global $wpdb;
