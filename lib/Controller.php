...
abstract class Controller
{
    ...
    protected function hasAccess( $action )
    {
        ...
        switch ( $security ) {
            case 'admin'     : return Utils\Common::isCurrentUserAdmin();
    ...
    protected function render( $template, $variables = array(), $echo = true )
    {
        ...
        try {
            if ( is_readable( $this->getModuleDirectory() . '/templates/' . $template . '.php' ) ) {
                $template = $this->getModuleDirectory() . '/templates/' . $template;
            }
    ...
    private function _enqueue( $type, array $sources )
    {
        ...
        foreach ( $sources as $source => $files ) {
            ...
            foreach ( $files as $key => $value ) {
                ...
                if ( $path === false ) {
                    call_user_func( $func, $file, false, $deps, Plugin::version() );
                } else {
                    call_user_func( $func, 'ab-' . basename( $file ), plugins_url( $file, $path ), $deps, Plugin::version() );
                }
            }
        }
    }
